...
1.  **仕様定義 (`spec-actions.md`)**:
    - 新機能や修正に着手する前に、まず各ゲームの`spec-actions.md`に、無効な操作の仕様を定義する。
    - 仕様には、どのような操作が無効であり、その際にどのような`console.error`メッセージが出力されるべきかを明記する。

2.  **E2Eテストによる仕様の検証**:
    - 次に、`spec-actions.md`に定義した仕様を検証するためのE2Eテストを先行して実装する。
