...
- **盤面外のUIコントロール (例: 「コマを確定する」ボタン、ターン進行ボタン) および一時的な無効状態:**
  - UIコントロールは、その操作の前提条件が満たされていない場合、`disabled`属性を`true`に設定し、ユーザーがクリックできないように明確に制御する必要があります。
    - **例1（前提条件）:** 「棒消しゲーム」において、まだ1本も棒が選択されていない状態では、「棒を消す」ボタンは`disabled`でなければなりません。
  - 同様に、ゲームの一時的な状態（例: アニメーション中、サーバー通信中、AI思考中）において、ユーザーの操作を受け付けられない盤面要素は、`disabled`としてユーザーにフィードバックすることが推奨されます。
    - **例2（一時的状態）:** 「神経衰弱」において、2枚のカードがめくられ、ペアが成立したかどうかを評価している最中は、他のカードをクリックできないように`disabled`にします。

この区別により、ユーザーは盤面を自由に試行錯誤できる一方で、明確なアクションを伴うボタン操作や、システムが処理中の状況では、意図しないエラーを防ぎ、UIの状態を直感的に理解できるようになります。
