...
ゲームの「おしえて！」（ヒント）機能は、プレイヤーの学習とゲーム体験の向上を目的として、一貫性のある設計原則に基づいて実装される必要があります。

#### 2.5.1. ヒントの仕様定義
各ゲームで提供されるヒント機能の具体的な仕様は、各ゲームディレクトリに配置された `spec-hint.md` にて定義されます。この仕様書は、そのゲームで利用可能な全てのヒントの種類、レベル、内容を網羅する責務を負います。

#### 2.5.2. ヒントのレベル
ヒントは、その性質に応じて以下の2つのレベルに分類されます。これにより、プレイヤーは必要に応じて情報の詳しさを選択できます。

-   **基本ヒント (`level: 'base'`)**
    -   **目的:** ゲームの基本的な操作を補助するための、常に表示される情報。
    -   **表示条件:** ヒント機能のON/OFF設定に関わらず、常に表示されます。
    -   **例:** 「リバーシで石を置ける場所」など、ルール上合法な手を視覚的に示すもの。

-   **戦略ヒント (`level: 'strategy'`)**
    -   **目的:** より有利にゲームを進めるための、戦略的なアドバイス。
    -   **表示条件:** プレイヤーが「おしえて！」機能をONにした場合にのみ表示されます。
    -   **例:** 「リバーシで最も多くの石を獲得できる手」「○×ゲームのリーチマス」など、次の手の良し悪しを評価する情報。

全てのゲームで両方のレベルのヒントを提供する必要はありません。ゲームの特性に応じて、適切なレベルのヒントを実装してください（例：○×ゲームには戦略ヒントのみが存在）。

#### 2.5.3. 仕様書の構造
各 `spec-hint.md` ファイルは、以下の情報を含む、一貫した構造で記述される必要があります。

-   **ヒントID (ID):** システム内部でヒントを一意に識別するためのID。
-   **レベル (Level):** 上記で定義された `base` または `strategy`。
-   **タイトル (Title):** UI上でユーザーに表示されるヒントの名称。
-   **説明 (Description):** ヒントがどのような情報を提供するのかを簡潔に説明する文章。
-   **表示仕様 (Display Specification):** そのヒントが画面上でどのように視覚的に表現されるかの具体的な説明。

#### 2.5.4. `manifest.json` との連携
`manifest.json` の `hints` プロパティは、`spec-hint.md` で定義されたヒントの中から、実際にそのゲームで有効化するヒントを設定するために使用します。これにより、コードを変更することなく、提供するヒントを柔軟に調整できます。
