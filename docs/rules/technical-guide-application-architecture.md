...
ユーザーによる無効な操作の扱いについては、操作の種類に応じてUIの応答を明確に区別します。

- **盤面上での直接操作 (例: コマの移動、セルのクリック):**
  - UI層（Reactコンポーネント）は、操作がルール上有効かどうかの事前チェックを行ってはなりません。`disabled`属性やイベントハンドラ内の条件分岐でクリックをブロックせず、すべての盤面インタラクションをコアロジック層 (`core.ts`) に委譲してください。
  - コアロジック層は、無効な操作を検知した場合、ゲーム状態を変更せずに`console.error`で適切なエラーメッセージを出力します。

- **盤面外のUIコントロール (例: 「コマを確定する」ボタン、ターン進行ボタン):**
  - これらのコントロールは、操作が不可能である（前提条件が満たされていない）状態では、`disabled`属性を`true`に設定し、ユーザーがクリックできないように明確に制御する必要があります。
  - 例えば、「棒消しゲーム」において、まだ1本も棒が選択されていない状態では、「棒を消す」ボタンは`disabled`でなければなりません。

この区別により、ユーザーは盤面を自由に試行錯誤できる一方で、明確なアクションを伴うボタン操作では、意図しないエラーを防ぐことができます。
...
- **例:**
    ...
    - はさみ将棋で「選択中のコマ」や「移動可能なマス」をハイライトする。
