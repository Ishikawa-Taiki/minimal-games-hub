...
2.  各ライブラリのライセンス情報を確認し、`public/licenses.txt` に追記します。
    **追記の際は、必ず以下の手順を厳守してください。**
    1.  `read_file` で `public/licenses.txt` の現在の全内容を読み込む。
    2.  読み込んだ内容の末尾に、新しく記録するライセンス情報を文字列として結合する。
    3.  `write_file` を使用し、結合後の文字列でファイル全体を上書きする。
    **ファイルの直接的な上書きによる既存情報の消失は、重大なルール違反とみなします。**
...
-   **ローカルでの事前確認:** 変更をコミットする前に、必ず以下のコマンドを実行し、ビルド、リンティング、型チェック、**およびテスト**が正常に完了することを確認しなければなりません。
    ```bash
    npm run build
    npm run lint
    npm test
    ```
-   **コミット前フック:** `husky` と `lint-staged` を使用して、コミット時に変更されたファイルに関連するテスト、リンティング、ビルドが自動的に実行されるように設定されています。これにより、テストやリンティング、ビルドが失敗するコードのコミットを防ぎます。
-   **CI/CD連携:**
...
5.  **変更のステージングとコミット:**
    ...
    *   変更されたファイルを `git add` でステージングします。この際、**    変更されたファイルを `git add` でステージングします。この際、**コミットに含めるべきファイルのみ**を正確にステージングします。一時ファイルや生成ファイルは含めません。
    特に、`docs/ai-workflow/3-session-context.md` と `docs/ai-workflow/logs/system.log` は、セッション中に変更された場合、**必ず関連するコード変更と同じコミットに含める**こと。**を正確にステージングします。一時ファイルや生成ファイルは含めません。
