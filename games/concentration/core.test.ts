...
describe('神経衰弱ゲームのコアロジック', () => {
  ...
  describe('handleCardClick', () => {
    ...
    it('2枚のカードがマッチした時（同じ数字）、スコアが加算され、カードはマッチ状態になる', () => {
      let state = createInitialState();

      // 同じランク（例: '01'）でスーツが異なる2枚のカードを見つける
      const firstCardIndex = state.board.findIndex(c => c.rank === '01');
      const secondCardIndex = state.board.findIndex((c, i) => c.rank === '01' && i !== firstCardIndex);

      state = handleCardClick(state, firstCardIndex);
      state = handleCardClick(state, secondCardIndex);

      expect(state.scores.player1).toBe(1);
      expect(state.board[firstCardIndex].isMatched).toBe(true);
      expect(state.board[secondCardIndex].isMatched).toBe(true);
      expect(state.flippedIndices.length).toBe(0);
