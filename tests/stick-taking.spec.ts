...
test.describe('棒消しゲーム', () => {
  ...
  test('難易度を選択するとゲームが開始されること', async ({ page }) => {
    ...
    await expect(page.getByTestId('game-state-display').locator('p')).toHaveText('プレイヤー1のばん');
  ...
  test('棒を取るとターンが交代すること', async ({ page }) => {
    ...
    await expect(page.getByTestId('game-state-display').locator('p')).toHaveText('プレイヤー2のばん');
