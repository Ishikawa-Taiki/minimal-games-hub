...
**ユーザーストーリー:** システム設計者として、デバッグ用の詳細なアクション履歴と、ユーザー向けのゲーム進行履歴を適切に分離したい。これにより、それぞれの目的に最適化された履歴管理が実現できる。

#### 受入基準

1. WHEN ゲーム進行アクション（手を打つ、リセット等）が実行される時 THEN ユーザー履歴とデバッグ履歴の両方に記録される SHALL
2. WHEN 内部アクション（ヒント表示、UI状態変更等）が実行される時 THEN デバッグ履歴のみに記録される SHALL
3. WHEN ユーザーがタイムトラベル操作を行う時 THEN ゲーム進行アクションの粒度で履歴を移動する SHALL
4. WHEN 開発者がデバッグを行う時 THEN 全ての内部アクションを含む詳細な履歴を確認できる SHALL
5. WHEN アクション型を定義する時 THEN ゲーム進行アクションと内部アクションが型レベルで区別される SHALL
6. WHEN 履歴エンジンを実装する時 THEN 両方の履歴が独立して管理される SHALL

### 要件10: 型安全性とインターフェース合成
...
6. WHEN アクション階層を型定義する時 THEN ゲーム進行アクションと内部アクションの関係が厳密に表現される SHALL