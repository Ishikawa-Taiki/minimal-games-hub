...
test.describe("リバーシゲームのE2Eテスト", () => {
  ...
  test("初期状態の盤面が正しく表示される", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="cell-3-4"] > div').first(),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="cell-3-4"] > div').first(),
    ).toHaveCSS("background-color", "rgb(0, 0, 0)");
    await expect(
      page.locator('[data-testid="cell-4-3"] > div').first(),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="cell-4-3"] > div').first(),
    ).toHaveCSS("background-color", "rgb(0, 0, 0)");
    await expect(
      page.locator('[data-testid="cell-4-4"] > div').first(),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="cell-4-4"] > div').first(),
    ).toHaveCSS("background-color", "rgb(255, 255, 255)");
    ...
    // スコアの初期状態を検証
    const blackScore = await page
      .locator('[data-testid="score-black"]')
      .textContent();
    expect(blackScore).toBe("2");
    const whiteScore = await page
      .locator('[data-testid="score-white"]')
      .textContent();
  ...
  test("履歴機能が正しく動作する", async ({ page }) => {
    // 初期状態の履歴カウンターを確認
    const initialHistoryCounter = await page
    ...
    await expect(
      page.locator('[data-testid="history-first-button"]'),
    ).toBeDisabled();
    await expect(
      page.locator('[data-testid="history-back-button"]'),
    ).toBeDisabled();
    await expect(
      page.locator('[data-testid="history-forward-button"]'),
    ).toBeDisabled();
    await expect(
      page.locator('[data-testid="history-last-button"]'),
    ).toBeDisabled();
    ...
    // 履歴カウンターが更新される
    const historyCounter1 = await page
      .locator('[data-testid="history-counter"]')
      .textContent();
    ...
    // 履歴カウンターが更新される
    const historyCounter2 = await page
      .locator('[data-testid="history-counter"]')
      .textContent();
    ...
    // 履歴カウンターが更新される
    const historyCounter3 = await page
      .locator('[data-testid="history-counter"]')
      .textContent();
    ...
    // 履歴カウンターが初期状態に戻る
    const historyCounter4 = await page
      .locator('[data-testid="history-counter"]')
      .textContent();
    ...
    // 履歴カウンターが最新状態になる
    const historyCounter5 = await page
      .locator('[data-testid="history-counter"]')
      .textContent();
  ...
  test.describe("無効な操作", () => {
    ...
    test("既に石が置かれているマスをクリックするとエラーになる", async ({
      ...
      // (3,3)には既に石が置かれている
      await page.locator('[data-testid="cell-3-3"]').click();
