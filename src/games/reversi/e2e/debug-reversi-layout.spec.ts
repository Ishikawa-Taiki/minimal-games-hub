...
test.describe("Reversi GameLayout Debug", () => {
  test("should render GameLayout with responsive behavior", async ({
    page,
  }) => {
    // リバーシページに移動
    ...
    console.log(
      "GameStateLogger logs:",
      logs.length > 0 ? logs.slice(0, 5) : "No logs found",
    );
  });
  ...
  test("should show mobile layout on small screen", async ({ page }) => {
    ...
    const fab = page
      ...
      .or(page.locator('button[aria-label*="コントロール"]'));
  ...
  test("should log game state changes", async ({ page }) => {
    ...
    const gameStateLoggerLogs = consoleLogs.filter(
      (log) =>
    ...
    // 操作後のログ数をチェック
    const finalGameStateLoggerLogs = consoleLogs.filter(
      (log) =>
        log.includes("GameStateLogger") ||
        log.includes("useReversi") ||
        log.includes("GameLayout"),
    ...
    console.log(
      "Final GameStateLogger related logs:",
      finalGameStateLoggerLogs.length,
    );

    // ログが増加していることを確認
    expect(finalGameStateLoggerLogs.length).toBeGreaterThan(
      gameStateLoggerLogs.length,
    );
  });
});
