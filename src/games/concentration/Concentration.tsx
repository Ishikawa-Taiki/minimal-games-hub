...
const Concentration = ({ controller }: ConcentrationProps) => {
  ...
  useEffect(() => {
    const styleId = 'concentration-animations';
      ...
      styleElement.innerHTML = `
        @keyframes pulse {
          0% {
            ...
            box-shadow: 0 0 10px 4px rgba(76, 175, 80, 0.7);
          }
          50% {
            transform: scale(1.1);
    ...
    // No cleanup needed, as the style can be shared across multiple instances or re-renders.
  }, []);
  ...
  const { difficulty, board, hintedIndices, newlyMatchedIndices } = gameState;
    ...
    return (
      ...
      >
        <div style={cardInnerStyle}>
          <div style={cardFaceStyle} data-testid={`card-face-${index}`}>
            <CardFaceContent suit={card.suit} rank={card.rank} />
          </div>
          <div style={cardBackStyle} data-testid={`card-back-${index}`} />
        </div>
      </div>
    );
  };
  ...
  const getBoardDimensions = () => {
    if (!difficulty) return {}; // 難易度未選択時はスタイルを適用しない
